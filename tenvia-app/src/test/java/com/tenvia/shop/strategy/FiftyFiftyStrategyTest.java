...
class FiftyFiftyStrategyTest {
    ...
    void canApply_expectException_whenSessionIsOver() {
        ...
        assertThrows(GameSessionOverException.class, () -> fiftyFiftyStrategy.apply(sessionId));
    }
    ...
    void canApply_expectTwoSelectedOptionIds() {
        ...
        AppliedEffectResult result = fiftyFiftyStrategy.apply(sessionId);
        List<QuestionOptionDTO> unavailableOptions = result.questionResponse().options().stream()
                .filter(opt -> !opt.isAvailable())
                .toList();

        assertThat(unavailableOptions.size()).isEqualTo(2);
    }
    ...
    void canApply_expectException_whenMaxUsageReached() {
        ...
        assertThrows(IllegalStateException.class, () -> fiftyFiftyStrategy.apply(sessionId));
    }

    @Test
    void canGetPowerUpType() {
        assertThat(fiftyFiftyStrategy.getPowerUpType()).isEqualTo(PowerUpType.FIFTY_FIFTY);
    }
